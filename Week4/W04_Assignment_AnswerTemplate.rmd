...
MY_STUDENT_ID <- 2718589 # <-- Replace "ABCDEFG" by your student ID number (as an integer, i.e., without quotes).
```
...
df_xy_imputed<- df_xy_missing %>%
  mutate(y = impute_by_median(unlist(select(df_xy_missing, y))))
head(df_xy_imputed)
...
wins_tidy <- wins_data_frame %>%
  rename(Team = ...1) %>%
  pivot_longer(-Team, names_to = "Year", values_to = "Result") %>%
  separate(Result, into = c("Wins", "Total"), sep = "of") %>%
  mutate(across(c(Year, Wins, Total), as.integer))
...
losses_tidy <- losses_data_frame %>%
  rename(Team = ...1) %>%
  pivot_longer(-Team, names_to = "Year", values_to = "Result") %>%
  separate(Result, into = c("Losses", "Total"), sep = "of") %>%
  mutate(across(c(Year, Losses, Total), as.integer))

losses_tidy %>% print(n = 4)
```
...
hockey_df <- left_join(wins_tidy, losses_tidy) %>%
  ...
  select(Team, Year, Wins, Losses, Total, Draws, Wins_rt, Losses_rt, Draw_rt)
...
summary_teams <- hockey_df %>%
  group_by(Team) %>%
  summarise(
    median_win_rt = median(Wins_rt),
    mean_win_rt = mean(Wins_rt),
    median_loss_rt = median(Losses_rt),
    mean_loss_rt = mean(Losses_rt)
  ) %>%
...
ci_mean_known_var(xtest2, known_var, 0.95)
```
...
ci_variance <- function(x, conf.level){
  ...
  s2 <- var(x)
  alpha <- 1 - conf.level
  c_lower <- (n - 1) * s2 / qchisq(1 - alpha/2, df = n-1)
  c_upper <- (n - 1) * s2 / qchisq(alpha/2, df = n-1)
  return(c(c_lower, c_upper))
}
ci_variance(xtest2, 0.95)
```
...
ci_mean_t <- function(x, conf.level){
  ...
  mean_x <- mean(x)
  se <- sd(x) / sqrt(n)
  t_val <- qt(1 - (1 - conf.level)/2, df = n-1)
  return(c(mean_x - t_val * se, mean_x + t_val * se))
}
ci_mean_t(xtest2, 0.95)
...
df_ci <-data.frame(
  ...
  mutate(
    CI.hw.norm = map_dbl(N, ~{
    x <- rnorm(.x)
    ci <- ci_mean_known_var(x, var = 1, 0.95)
    diff(ci) / 2
  }),
    CI.hw.t = map_dbl(N, ~{
    x <- rnorm(.x)
...
ggplot(df_ci, aes(x = N)) + geom_line(aes(y = CI.hw.norm, color = "Know variance")) +
  geom_point(aes(y = CI.hw.norm), shape = 16, size = 2, color = "red") +
  geom_line(aes(y = CI.hw.t, color = "Unknow variance "))+
  geom_point(aes(y = CI.hw.t), shape = 2, size = 2, color = "skyblue") +
  theme_bw() +
  labs(x = "N", y ="CL half-width")
...
median_boot_ci <- function(x, R = 2000, conf = 0.95) {
  set.seed(123)
  n <- length(x)
  boot_meds <- replicate(R, median(sample(x, size = n, replace = TRUE)))
  alpha <- 1 - conf
  quantile(boot_meds, probs = c(alpha/2, 1 - alpha/2))
}
```
...
oecd.pharma <- read_csv("pharma_drug_spending.csv")
usd2022 <- oecd.pharma %>%
  filter(TIME == 2022) %>%
  pull(USD_CAP)

# med_usd_cap_ci <- ...
med_usd_cap_ci <- median_boot_ci(usd2022, R = 2000, conf = 0.95)
med_usd_cap_ci
...
```{r Q4c}
## Question 4.c
boot_summary <- oecd.pharma %>%
  filter(TIME >= 1980 & TIME <= 2020) %>%
  group_by(TIME) %>%
  summarise(
    boot_ci = list(median_boot_ci(PC_GDP, R = 2000, conf = 0.95)),
    median_est = median(PC_GDP)
  ) %>%
  unnest_wider(boot_ci, names_sep = "_")

ggplot(boot_summary, aes(x = TIME, y = median_est)) +
  geom_line(color = "blue") +
  geom_ribbon(aes(ymin = boot_ci_1, ymax = boot_ci_2), alpha = 0.2, fill = "skyblue") +
  theme_bw() +
  labs(x = "Year", y = "Median % GDP (Pharma)", 
       title = "Bootstrap 95% CI on Median %GDP for Pharma Drugs (1980–2020)")
