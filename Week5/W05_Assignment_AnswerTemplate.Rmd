...
myN <- ceiling(power.t.test(delta = 0.01, sd = 0.05,
...
q5.df.summ <- readRDS("UPMSP_SA_full_results.rds") %>%
...
```{r Q5e}
## Question 5.e
mytest.full <- t.test(q5.df.summ $ Mean.MS,
                      mu = 240,
                      alternative = "less",
                      conf.level = 0.95)
...
q5.parametric.p <- mytest.full$p.value
q5.parametric.p
```
...
t0 <- mytest.full$statistic

Xc <- X - mean(X) + mu_0

# Bootstrap
t0_star <- replicate(nboot, {
  tmp <- sample(Xc, replace = TRUE)
  t.test(tmp, mu = mu_0)$statistic
})

p.boot <- mean(t0_star < t0)
p.text <- ifelse(p.boot == 0, paste0("p < ", 1/nboot),
                 paste0("p = ", round(p.boot, 4)))
p.text


ggplot(data.frame(x = t0_star), aes(x = x)) +
  geom_histogram(fill = "lightblue", bins = 30) +
  geom_vline(xintercept = t0, color = "red", linetype = "dashed") +
  ggtitle("Bootstrap t-test",
          subtitle = paste("Observed t0 =", round(t0,2), ",", p.text)) +
  xlab("Bootstrap t0") + theme_minimal()
